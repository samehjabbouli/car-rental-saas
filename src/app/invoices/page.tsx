...
export default function InvoicesPage() {
  ...
  async function fetchInvoices(token: string) {
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/invoices?select=*&order=created_at.desc&limit=100`, {
        headers: {
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500">إدارة الفواتير والمدفوعات</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">+ إنشاء فاتورة</button>
        </div>
            ...
            <tbody>
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-b hover:bg-slate-50">
                  <td className="py-3 px-4 font-mono">#{inv.id.slice(0, 8)}</td>
                  <td className="py-3 px-4">{inv.total_amount || '-'} ر.س</td>
